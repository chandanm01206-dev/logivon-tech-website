import { getRepos, fallbackRepos } from '@/lib/github';
import ProjectCard from './ProjectCard';
import { ArrowUpRight } from 'lucide-react';

export default async function Work() {
    const { repos, error } = await getRepos();
    const list = repos && repos.length > 0 ? repos : fallbackRepos;
    const usingFallback = !repos || repos.length === 0;
                ...
                <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6 mb-12">
                    <div>
                        ...
                        <p className="text-white/50 mt-3 max-w-lg">
                            Live, open-source projects — pulled straight from our GitHub.
                        </p>
                    </div>
                    {!usingFallback && (
